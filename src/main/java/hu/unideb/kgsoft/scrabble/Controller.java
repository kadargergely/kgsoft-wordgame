 ...
 * These classes are:
 * <ul>
 * <li>{@link Dictionary}: contains the list of the words that can be formed.
 * </li>
...
public class Controller {
	 ...
	 */
	public void handleMousePressedOnBoard(int row, int col, double mouseX, double mouseY) {
		if (playersTurn) {
			if (player.getTileInHand() == -1) {
				...
				if (player.getTileInHand() == Letters.JOKER_CODE) {
					gameBoard.getFields()[row][col].setJokerTileCode(Letters.getC(view.getLetterFromUser()));
	...
	public void computerReady() {
		...
		if (playedWords == null) {
			if (bag.getNumOfTiles() == 0) {
				...
				numOfPasses++;
				logger.info(String.format("Number of subsequent passes is %d", numOfPasses));
				checkGameEnd();
			} else {
				numOfPasses = 0;
	...
	public void handleDoneButton() {
		logger.info(String.format("%s finished his/her turn by placing tiles on the board.", playerName));
		numOfPasses = 0;
		playersTurn = false;
		if (firstTurn) {
			firstTurn = false;
	...
	private void checkGameEnd() {
		if (numOfPasses == 4) {
			logger.info(String.format("Game ended. %s won.",
					player.getScore() >= computer.getScore() ? playerName : "Computer"));
			...
			for (int j = 0; j < Gameboard.BOARD_SIZE; j++) {
				if (gameBoard.getFields()[i][j].getTileCode() != Field.NOTILE) {
					if (gameBoard.getFields()[i][j].getTileCode() == Letters.JOKER_CODE) {
						view.drawTileOnBoard(gameBoard.getFields()[i][j].getJokerTileCode() + Letters.getNum(), i, j);
					} else {
						view.drawTileOnBoard(gameBoard.getFields()[i][j].getTileCode(), i, j);
					}
				}
			}
		}
	...
	public boolean login(String username, String passwd) {
		try {
			if (UserDAO.authenticateUser(username, passwd)) {
				...
				logger.info("Couldn't log in! Wrong user name or password.");
				loginView.showErrorMessage("Hiba a bejelentkezésnél!", "Hibás felhasználónév vagy jelszó.");
				return false;
			}
		} catch (DBConnectionException e) {
			loginView.showErrorMessage("Kapcsolódási hiba",
					"Nem sikerült elérni az adatbázis szervert. Játszhatsz offline, de nem lehet majd játékot menteni vagy betölteni.");
		}
		...
		try {
			if (passwd.equals(passwdAgain)) {
				if (UserDAO.registerUser(username, passwd)) {
					...
					loginView.showErrorMessage("Nem lehet léltrehozni az új felhasználót!",
				...
				loginView.showErrorMessage("Elírtad a jelszót.", "A két jelszó mező tartalma nem egyezik.");
				return false;
			}
		} catch (ExistingUserException e) {
			logger.info(String.format("Couldn't register new user. User name '%s' already exists.", username));
			loginView.showErrorMessage("Nem lehet léltrehozni az új felhasználót!", "A felhasználónév már létezik.");
			return false;
		} catch (DBConnectionException e) {
			loginView.showErrorMessage("Kapcsolódási hiba", "Nem sikerült elérni az adatbázis szervert.");
		}
	...
	public void loadGame() {
		...
		try {
			...
			view.showErrorMessage("Kapcsolódási hiba", "Nem sikerült elérni az adatbázis szervert.");
		}
		if (savedGame != null) {
			gameBoard = new Gameboard(savedGame.getFieldsAsTwoDimensionalArray());
			gameBoard.setMultipliers();
			player.setScore(savedGame.getPlayerScore());
			computer.setScore(savedGame.getComputerScore());
			bag = new Bag(savedGame.getBagRemainingAsArray());
			player.getTray().setLetters(savedGame.getPlayerTrayTileCodes());
			player.setTileInHand(savedGame.getTileInHand());
			computer.getTray().setLetters(savedGame.getComputerTrayTileCodes());
			...
			gameState.setGameStarted(gameStarted);
			gameState.setPlayerName(playerName);
			gameState.setLastWords(Arrays.asList(savedGame.getLastWords().split(" ")));
	 ...
	 */
	public void saveGame() throws DBConnectionException {
		...
		SavedGame savedGame = new SavedGame(0, null, playerTray, computerTray, player.getTileInHand(), playersTurn,
				player.getScore(), computer.getScore(), gameState.getLastWords(), gameState.getLastWordsPoints(),
				bag.getRemaining(), gameBoard.getFields());
